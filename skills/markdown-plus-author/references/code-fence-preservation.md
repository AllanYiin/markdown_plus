...
### 2e. DSL fence
`mermaid` / `svg` / `vega-lite` / `plotly` / `plantuml` 等已經是合法 markdown 慣例的 DSL fence,內容是該語言原文 → 整段保留。

對 ` ```svg ` 與 ` ```mermaid `,Markdown+ viewer 會額外提供 `graph` / `code` 雙視圖 toggle:
- ` ```svg ` ≤ 16 KB → 內聯渲染向量圖;> 16 KB 顯示警告並要求外移成 `.svg`
- ` ```mermaid ` → 動態載入 Mermaid 10 渲染為 SVG;viewer 同時抽 fence 第一行的圖表類型(`sequenceDiagram` / `flowchart` / `classDiagram` / `gantt` / `graph` / ...)當 block 的結構關鍵詞

但這層 viewer 行為**不影響本檔的核心規則**:fence 內容仍要逐字保留、不允許重排或拍扁。

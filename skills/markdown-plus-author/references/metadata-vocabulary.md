...
### Discoverability (對 AI agent 查詢)
| Key | 規則 |
|---|---|
| `keywords` | `[a,b,c]` 或 `a,b,c` — block 關鍵詞清單。**預設留空**,viewer 會用無辭典演算法(N-gram + PMI + 左右熵)自動抽取並以虛線 chip 顯示;手動宣告會取代 auto 並以實線 chip 呈現。詳見 `syntax-reference.md` §16。不要把已自動偵測的結構標籤(`svg` / `mermaid` / mermaid 圖表類型)再手寫進去——重複而已 |
